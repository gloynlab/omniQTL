...
class EGAsubmitter:
    def encrypt_files_for_ega(self, in_dir='fastq', pubkey='ingestion.pubkey', out_file='encrypt.sh'):
    ...
    def get_sample_run_tables(self, in_dir='fastq', sample_out_file='samples.csv', run_out_file='runs.csv', prefix='rnaseq', sample_header=['alias', 'title', 'description', 'biological_sex', 'subject_id', 'phenotype', 'biosample_id', 'case_control', 'organism_part', 'cell_line']):
        ...
        for f in os.listdir(in_dir):
            if f.endswith('.fq.gz') or f.endswith('.fastq.gz'):
                sample = f.split('.fastq')[0].split('.fq')[0]
                sample = sample.split('_R1')[0].split('_R2')[0].split('_1')[0].split('_2')[0]
                D.setdefault(sample, [])
                D[sample].append(f)
            elif f.endswith('.bam'):
        ...
        df_sample = pd.DataFrame(columns=sample_header)
        df_sample['subject_id'] = samples
        df_sample['alias'] = prefix + '_' + df_sample['subject_id']
        df_sample.fillna('unknown', inplace=True)
        df_sample.to_csv(sample_out_file, index=False)

        L = []
        for sample in samples:
            alias = prefix + '_' + sample
            files = sorted(D[sample])
            if len(files) == 2:
                L.append([alias, files[0], files[1]])
            elif len(files) == 1:
                L.append([alias, files[0]])
        df_run = pd.DataFrame(L)
        if df_run.shape[1] == 2:
            df_run.columns = ['alias', 'file']
        elif df_run.shape[1] == 3:
            df_run.columns = ['alias', 'file1', 'file2']
        df_run.to_csv(run_out_file, index=False)
